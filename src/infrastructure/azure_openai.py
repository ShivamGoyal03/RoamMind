...
class AzureOpenAIService:
    ...
    async def enhance_excursion_details(self, api_response: Dict) -> Dict:
        ...
        prompt = self._create_enhancement_prompt(api_response, enhancement_type="excursion details")
    ...
    async def extract_hotel_information(self, input_text: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            prompt = self._create_extraction_prompt("hotel", input_text, context)
            response = await self._call_openai(prompt)
    ...
    async def extract_restaurant_information(self, input_text: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            prompt = self._create_extraction_prompt("restaurant", input_text, context)
            response = await self._call_openai(prompt)
    ...
    async def extract_excursion_information(self, input_text: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            prompt = self._create_extraction_prompt("excursion", input_text, context)
            response = await self._call_openai(prompt)
    ...
    def _create_extraction_prompt(self, type: str, input_text: str, context: Dict[str, Any]) -> str:
        ...
        if context.get("user_preferences"):
            base_prompt += f"\nConsider user preferences: {context['user_preferences']}"
    ...
    def _create_enhancement_prompt(self, data: Any, context: Optional[Dict[str, Any]] = None, enhancement_type: str = "details") -> str:
        """
        Construct a prompt for LLM to enhance the flight data.
        
        Parameters:
          data: The raw flight data (search results, details, availability).
        ...
        """
        prompt = f"Enhance the following flight {enhancement_type} information to be more clear, detailed, and user-friendly. Please return valid JSON.\n"
    ...
    def _parse_enhancement_response(self, response: str) -> Any:
        try:
            ...
            # Add schema validation here if you have a JSON schema
            return data
    ...
    async def enhance_hotel_results(self, hotels: List[Dict], search_params: Dict) -> List[Dict]:
        try:
            prompt = self._create_enhancement_prompt(hotels, context=search_params)
            response = await self._call_openai(prompt)
    ...
    async def enhance_hotel_details(self, hotel_data: Dict) -> Dict:
        ...
        prompt = self._create_enhancement_prompt(hotel_data, enhancement_type="hotel details")
        response = await self._call_openai(prompt)
    ...
    async def enhance_hotel_availability(
        ...
        context = {"hotel_id": hotel_id, "room_id": room_id, "check_in": check_in.isoformat(), "check_out": check_out.isoformat(), "guests": guests}
        prompt = self._create_enhancement_prompt(availability, context=context, enhancement_type="hotel availability")
        response = await self._call_openai(prompt)
        enhanced = self._parse_enhancement_response(response)
        if isinstance(enhanced, dict):
            return enhanced
    ...
    def _create_validation_prompt(self, excursion_id: str, date: datetime, participants: int, customer_details: Dict) -> str:
        ...
        {{"is_valid": true}}  //Example of valid response
        {{"is_valid": false, "reason": "The excursion is not available on the selected date."}} //Example of invalid response
    ...
    async def enhance_excursion_results(self, excursions: List[Dict], search_params: Dict) -> List[Dict]:
        ...
        """
        prompt = self._create_enhancement_prompt(excursions, context=search_params, enhancement_type="excursion search results")
        response = await self._call_openai(prompt)
        enhanced = self._parse_enhancement_response(response)
    ...
    async def enhance_excursion_categories(self, categories: List[str], location: str) -> List[str]:
        ...
        prompt = f"Given the location {location}, enhance the following excursion categories: {json.dumps(categories)}. Return the result as a JSON list."
        response = await self._call_openai(prompt)
        try:
            ...
            logger.error("Failed to parse enhancement response for excursion categories.")
        return categories
    ...
    async def enhance_flight_results(self, flights: List[Dict], search_params: Dict) -> List[Dict]:
        try:
            prompt = self._create_flight_results_enhancement_prompt(flights, search_params)
            ...
            logger.info(f"Enhanced Flight Results: {enhanced_flights}") #Log the enhanced results
            if not isinstance(enhanced_flights, list):
                logger.warning(f"LLM response is not a list: {enhanced_flights}")
                return flights  # Fallback
    ...
    async def enhance_flight_details(self, flight_data: Dict) -> Dict:
        ...
        Parameters:
          flight_data: A dictionary containing detailed flight information.
        
        Returns:
          An enhanced flight detail dictionary.
        """
        prompt = self._create_enhancement_prompt(flight_data, enhancement_type="details")
        response = await self._call_openai(prompt)
        enhanced = self._parse_enhancement_response(response)
        if isinstance(enhanced, dict):
            return enhanced
    ...
    async def enhance_flight_availability(
        ...
        Parameters:
          availability: A dictionary containing raw availability data.
          flight_id: The identifier of the flight.
          seats: Number of seats for which availability was checked.
          seat_class: (Optional) The cabin class (economy/business/etc.).
        
        Returns:
          An enhanced availability dictionary.
        """
        context = {"flight_id": flight_id, "seats": seats, "seat_class": seat_class}
        prompt = self._create_enhancement_prompt(availability, context=context, enhancement_type="availability")
        response = await self._call_openai(prompt)
        enhanced = self._parse_enhancement_response(response)
        if isinstance(enhanced, dict):
            return enhanced
    ...
    async def enhance_menu_details(self, menu_data: Dict, restaurant_id: str) -> Dict:
        ...
        prompt = self._create_enhancement_prompt(menu_data, context={"restaurant_id": restaurant_id}, enhancement_type="menu details")
        response = await self._call_openai(prompt)
        enhanced = self._parse_enhancement_response(response)
        if isinstance(enhanced, dict):
            return enhanced
    ...
    async def analyze_restaurant_reviews(self, reviews: List[Dict], restaurant_id: str) -> Dict:
        ...
        prompt = self._create_enhancement_prompt(reviews, context={"restaurant_id": restaurant_id}, enhancement_type="restaurant reviews")
        response = await self._call_openai(prompt)
        enhanced = self._parse_enhancement_response(response)
        if isinstance(enhanced, dict):
            return enhanced
        return {"reviews": reviews, "summary": "No analysis available."}
    ...
    async def enhance_restaurant_recommendations(self, recommendations: List[Dict], preferences: Optional[Dict] = None) -> List[Dict]:
        ...
        prompt = self._create_enhancement_prompt(recommendations, context=preferences, enhancement_type="restaurant recommendations")
        response = await self._call_openai(prompt)
        enhanced = self._parse_enhancement_response(response)
        if isinstance(enhanced, list):
            return enhanced
